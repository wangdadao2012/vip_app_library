...
//                上次更新失败返回""
    ...
    private void analysisDefaultJson(){
        try {
            ...
            String launchPage=object.getString("launchPage");
            ...
            globalConfig.setLaunchPage(launchPage);
            globalConfig.setPages(pages);
            globalConfig.save();

            //navigationBar
            JSONObject navigationBarObject=new JSONObject(object.getString("navigationBar"));
            String backgroundColor=navigationBarObject.getString("backgroundColor");
            String textColor=navigationBarObject.getString("textColor");
            //tabBar
            JSONObject tabBarObject=new JSONObject(object.getString("tabBar"));
            String color=tabBarObject.getString("color");
            String selectedColor=tabBarObject.getString("selectedColor");
            String position=tabBarObject.getString("position");
            List<TabList> tabList=new Gson().fromJson(tabBarObject.getString("list")
                    , new TypeToken<List<TabList>>() {
                    }.getType());
            DataSupport.deleteAll(TabList.class);
            DataSupport.saveAll(tabList);
            DataSupport.deleteAll(TabBar.class);
            TabBar tabBar=new TabBar();
            tabBar.setColor(color);
            tabBar.setSelectColor(selectedColor);
            tabBar.setPosition(position);
            tabBar.setTabLists(tabList);
            tabBar.save();

            DataSupport.deleteAll(PageConfig.class);
            PageConfig pageConfig=new PageConfig();
            pageConfig.setNavBarBgColor(backgroundColor);
            pageConfig.setNavBarTextColor(textColor);
...
//              把名称写入表
                ...
                htmlTags.save();
                //如果字符串末端两个字符包含js,说明是js文件，否则css，据此拼接html代码
                if (aHtmlTags.substring(aHtmlTags.length()-2,aHtmlTags.length()).contains("js")){
                    tagSB.append("<script src=\"").append(contentPath).append(aHtmlTags).append("\"></script>").append("\n");
                }else {
                    tagSB.append("<link href=\"").append(contentPath).append(aHtmlTags).append("\" rel=\"stylesheet\">").append("\n");
                }
            }
